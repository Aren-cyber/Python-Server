...
from smartyard_db import create_db_connection, db
...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    ...
    or f"postgresql://{os.getenv('PG_USER')}:{os.getenv('PG_PASS')}@{os.getenv('PG_HOST')}:5432/{os.getenv('PG_DBNAME')}"
)
...
def access_verification(key):
    global response
    # TODO: тут надо проверять реально в базе, что правда есть доступ - например, что оплачена подписка
    ...
    if not db.session.query(
        db.session.query(Users).filter_by(uuid=key.get("Authorization")[7:]).exists()
    ).scalar():
        response = {"code": 401, "name": "Не авторизован", "message": "Не авторизован"}
        abort(401)
    return (
        db.session.query(Users.userphone)
        .filter_by(uuid=key.get("Authorization")[7:])
        .first()[0]
    )
...
def accessfl():
    ...
    if db.session.query(db.exists().where(Users.videotoken == token)).scalar():
        row = [
            r._asdict()
            for r in db.session.query(Users.vttime, Users.strims)
            .filter_by(videotoken=token)
            .all()
        ]
        vttime = row[0]["vttime"]
        strims = row[0]["strims"]
    ...
    if not proxyfl:
        ...
        if response.status_code != 200:
            response = {
                "code": 403,
                "name": "Forbidden",
                "message": "И здесь не верный токен",
            }
            abort(403)
...
def paysuccess():
    ...
    with open("pub.pem", "r", encoding="utf-8") as f:
        ...
        if (
            (operation == "deposited" or operation == "declinedByTimeout")
            and status == "1"
            and verifier.verify(h, signature)
        ):
            row = [
                r._asdict()
                for r in db.session.query(Invoices.amount, Invoices.agrmid)
                .filter_by(invoice_id=orderNumber)
                .all()
            ]
            db.session.query(Invoices).filter_by(invoice_id=orderNumber).update(
                {"invoice_pay": True}
            )
            db.session.commit()
...
def cams_caminit():
    ...
    if "macaddress" in request_data:
        if db.session.query(
            ...
            .filter_by(device_mac=request_data["macaddress"])
            .exists()
        ).scalar():
            device_uuid = (
                db.session.query(Devices.device_uuid)
                .filter_by(device_mac=request_data["macaddress"])
                .first()[0]
            )
        else:
            ...
            new_cam = Devices(
                ...
                device_mac=request_data["macaddress"],
                device_type=2,
                affiliation=None,
                owner=None,
                url=None,
...
def address_getAddressList():
    ...
    doors0.append(
        {"domophoneId": "4", "doorId": 1, "icon": "wicket", "name": "Калитка"}
    )
    ...
    if len(addList) == 0:
        ...
        for itemd in addList:
            ...
            if (
                db.session.query(
                    db.session.query(Rights).filter_by(uid=uid).exists()
                ).scalar()
                and is_active == 1
            ):
                rights = []
                doors = []
                rights = (
                    db.session.query(Rights.uid_right).filter_by(uid=uid).first()[0]
                )
                for item in rights:
                    device_type = (
                        db.session.query(Devices.device_type)
                        .filter_by(device_id=item, is_active=True)
                        .first()[0]
                    )
                    ...
                    if device_type == 3:
                        ...
                        row = [
                            ...
                            for r in db.session.query(
                                Doors.id, Doors.icon, Doors.entrance, Doors.name
                            )
                            .filter_by(device_id=item)
                            .all()
                        ]
    ...
    if db.session.query(db.exists().where(Settings.uid == uid)).scalar():
        if "settings" in request_data:
            ...
            if "CMS" in settings:
                ...
                db.session.query(Settings).filter_by(uid=uid).update(
                    {"intercom": intercom}
                )
                db.session.commit()
            if "VoIP" in settings:
        ...
        "data": [
            {
                ...
                "event": "1",
                "detail": "1",
                "preview": "https:\/\/static.dm.ugtelecom.ru\/2021-12-15\/3\/f\/9\/9\/3f99bdf6-96ef-4300-b709-1f557806c65b.jpg",
                "previewType": 2,
                "detailX": {
                    "opened": "f",
                    "face": {
            ...
            },
            {
                ...
                "event": "4",
                "detail": "89103523377",
                "preview": "https:\/\/static.dm.ugtelecom.ru\/2021-12-15\/8\/6\/d\/d\/86ddb8e1-1122-4946-8495-a251b6320b99.jpg",
                "previewType": 1,
            ...
            },
            {
                ...
                "event": "4",
                "detail": "89103523377",
                "preview": "https:\/\/static.dm.ugtelecom.ru\/2021-12-15\/a\/d\/1\/4\/ad14c83a-126a-4f09-a659-f412fb11007e.jpg",
                "previewType": 1,
                "detailX": {"phone": "89103523377"},
            },
            {
                ...
                "event": "4",
                "detail": "89103523377",
                "preview": "https:\/\/static.dm.ugtelecom.ru\/2021-12-15\/0\/b\/3\/3\/0b335948-864f-41d6-b9a7-465f88f20ef1.jpg",
                "previewType": 1,
                "detailX": {"phone": "89103523377"},
            },
            {
                ...
                "event": "4",
                "detail": "89103523377",
                "preview": "https:\/\/static.dm.ugtelecom.ru\/2021-12-15\/8\/f\/c\/3\/8fc3224e-ef46-4ec6-9d5d-04e249ec2e31.jpg",
                "previewType": 1,
                "detailX": {"phone": "89103523377"},
            },
            {
                ...
                "event": "4",
                "detail": "89103523377",
                "preview": "https:\/\/static.dm.ugtelecom.ru\/2021-12-15\/c\/2\/8\/c\/c28c7e58-7797-4143-a2b8-2c513e216bb8.jpg",
                "previewType": 1,
                "detailX": {"phone": "89103523377"},
            },
        ],
...
def cctv_all():
    ...
    strims = []
    if db.session.query(
        db.session.query(Rights).filter_by(uid=houseId).exists()
    ).scalar():
        rights = []
        ...
        for right in rights:
            row = [
                ...
                .filter_by(device_id=right, is_active=True)
                .all()
            ]
...
def cctv_camMap():
    ...
    response = {
        ...
        "data": [
            {
                "id": "5",
                "url": "https://fl2.ugtelecom.ru:8443/91052",
...
def cctv_recDownload():
    ...
    fileurl = (
        "https://ssmp.ugtelecom.info/"
        + db.session.query(Records.fileurl).filter_by(id=recordid).first()[0]
    )
...
def cctv_recPrepare():
    ...
    row = [
        r._asdict()
        for r in db.session.query(Devices.device_uuid, Devices.url, Devices.title)
        .filter_by(device_id=cam_id, is_active=True)
        .all()
    ]
    if row:
    ...
    if int(time_rec.seconds) > 900:
        response = {
            "code": 403,
            "name": "Set less than 15 min.",
            "message": "Установите менее 15 мин.",
        }
        abort(403)

    row = [
        r._asdict()
        for r in db.session.query(Users.videotoken, Users.uid)
        .filter_by(userphone=phone)
        .all()
    ]
    ...
    url = (
        ...
        + "/"
        + "archive-"
        + str(int(time.mktime(time_from.timetuple())))
        + "-"
        + str(time_rec.seconds)
        + ".mp4?token="
        + "100"
        + str(row[0]["videotoken"])
    )
    filedir = "/www/"
    fileurl = (
        camname.lower().replace(",", "_").replace(".", "_").replace(" ", "_")
        + "_"
    ...
    db.session.commit()
    recordId = (
        db.session.query(Records.id).filter_by(url=url, fileurl=fileurl).first()[0]
    )
    resCode = 403
    with open(fullfileurl, "wb") as fil:
        cur = pycurl.Curl()
    ...
    print(
        f"resCode =   {resCode}    resText =   {resText} url = {url}    fullfileurl = {fullfileurl} !"
    )
    if resCode == 200:
...
def frs_listFaces():
    ...
    response = {
        ...
        "data": [
            {
                "faceId": "144349",
                "image": "https://faces.dm.ugtelecom.ru/3/9/9/c/d/399cd1a1-4545-2443-33de-a3dc8af728f7.jpg",
            }
        ],
    }
    return jsonify(response)
...
def geo_coder():
    ...
    response = {
        ...
        "name": "OK",
        "message": "Хорошо",
        "data": {"address": address, "lat": "52.609296", "lon": "39.59851"},
    }
    return jsonify(response)
...
def inbox_inbox():
    ...
    response = {
        ...
        "name": "OK",
        "message": "Хорошо",
        "data": {"basePath": "https:\/\/dm.ugtelecom.ru\/", "code": "Hello Word"},
    }
    return jsonify(response)
...
def pay_prepare():
    ...
    new_invoice = Invoices(
        ...
        invoice_time=None,
        invoice_pay=None,
        contract=request_data["clientId"],
    ...
    response = {
        ...
        "name": "OK",
        "message": "Хорошо",
        "data": str(new_invoice.invoice_id),
    }
    print(f"{response}")
    return jsonify(response)
...
def user_appVersion():
    ...
    if version is not None and (platform == "android" or platform == "ios"):
        db.session.query(Users).filter_by(userphone=int(phone)).update(
...
def user_confirmCode():
    ...
    request_data = json_verification(request)
    if (
        (not "userPhone" in request_data)
        or len(request_data["userPhone"]) != 11
        or (not "smsCode" in request_data)
        or len(request_data["smsCode"]) != 4
    ):
        response = {
            "code": 422,
            "name": "Unprocessable Entity",
                        ...
                        [
                            {
                                "clientId": "00000",
                                "clientName": "",
                                "contractName": "Здесь будет Ваш логин",
                                "blocked": "f",
                                "balance": 0,
                                "bonus": 0,
                                "contractPayName": "00000",
                                "payAdvice": 0,
                            ...
                            [
                                {
                                    "clientId": dogovor,
                                    "clientName": "",
                                    "contractName": dogovor,
                                    "blocked": "f",
                                    "balance": balance,
                                    "bonus": 0,
                                    "contractPayName": dogovor,
                                    "payAdvice": pay,
                                    "services": [{"internet", "cctv", "domophone"}],
                                }
                            ]
...
def user_notification():
    ...
    row = [
        r._asdict()
        for r in db.session.query(Users.money, Users.notify)
        .filter_by(userphone=int(phone))
        .all()
    ]
...
def user_registerPushToken():
    ...
    if "pushToken" in request_data:
        ...
        db.session.query(Users).filter_by(userphone=int(phone)).update(
            {"pushtoken": pushtoken, "platform": platform}
        )
...
def user_requestCode():
    ...
    if not user_phone == int(testuser):
        if sms_sender == "gammu":
            sendsms = subprocess.run(
                [
                    "gammu sendsms TEXT +7%s -unicode -report -text '%s'"
                    % (str(user_phone)[1:], sms_text)
                ],
                shell=True,
            )
        if sms_sender == "kannel":
            ...
            try:
                response = requests.get(
                    url=kannel_url, params=kannel_params + kannel_params2
                )
